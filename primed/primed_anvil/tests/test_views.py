...
class UserSearchAutocompleteTest(TestCase):
    def setUp(self):
        """Set up test class."""
        self.factory = RequestFactory()
        # Create a user with the correct permissions.
    ...
    def get_url(self, *args):
        """Get the url for the view being tested."""
        return reverse("primed_anvil:user:autocomplete", args=args)

    def get_view(self):
        """Return the view being tested."""
        return views.UserSearchAutocomplete.as_view()
    ...
    def test_returns_correct_object_containing_query(self):
        ...
        self.client.force_login(self.user)
        response = self.client.get(self.get_url(), {"q": "other"})
        returned_ids = [
            x["id"] for x in json.loads(response.content.decode("utf-8"))["results"]
        ]
        self.assertEqual(len(returned_ids), 1)
